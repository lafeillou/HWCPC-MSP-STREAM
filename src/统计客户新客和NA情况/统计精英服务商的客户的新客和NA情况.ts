...
export default (flag?: any) => {
  return new Promise(async (resolve, reject) => {
    ...
    console.time("统计精英服务商的客户的新客和NA情况");
    // 初始化数据库连接
    // await connection();

    const customers = await AppDataSource.getRepository(Customer)
      .createQueryBuilder("customer")
      .where("customer.customerType = :customerType", { customerType: 1 }) // 类型为精英服务商的客户
    ...
    for (let m = flag === "init" ? 1 : currentMonth; m <= currentMonth; m++) {
      ...
      for (let i = 0; i < customers.length; i++) {
        const c = customers[i];
        const p = await AppDataSource.getRepository(Partner)
          .createQueryBuilder("partner")
          .where("partner.isNewest = :isNewest", { isNewest: 1 }) // 最新版本的客户
          .andWhere("partner.indirect_partner_id = :indirect_partner_id", {
            indirect_partner_id: c.parent_id,
          })
          .getOne();
        // console.log(c);
        // console.log(p);
        const temp = {
          ...
          二级经销商账号名: p.account_name,
          二级经销商ID: p.indirect_partner_id,
          二级经销商名称: p.name,
          客户类型: "精服客户",
          月度: m,
          备注: null,
    ...
    console.timeEnd("统计精英服务商的客户的新客和NA情况");
    resolve("统计精英服务商的客户的新客和NA情况 done");
  });
};
