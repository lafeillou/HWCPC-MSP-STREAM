...
(async () => {
  ...
  new XLSX()
    .extract(__dirname + "/员工IAM账号2023-06.xlsx", { sheet_id: 1 }) // or sheet_name or sheet_nr
    // .on('sheet', (sheet) => {
    ...
    .on("row", async (row) => {
      if (rowIndex === 0) {
        ...
        // console.log(row);
        if (!row[headerMap["BP账号"]]) {
          return;
        }
        const record: any = {};
        record.bpName = _.trim(row[headerMap["BP账号"]]);
        // record.workerId = row[headerMap["工号"]];
        // record.area = row[headerMap["区域"]];
        record.name = _.trim(row[headerMap["姓名"]]);
        record.iamAccount = _.trim(row[headerMap["IAM账号"]]);
